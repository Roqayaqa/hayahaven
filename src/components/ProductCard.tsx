...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link href={`/product-details/${product.id}`} aria-label={`View details for ${product.name}`}>
      <Card
        className="w-full md:w-80 shadow-lg rounded-3xl overflow-hidden transition-transform duration-300 hover:scale-105 cursor-pointer"
        ...
        <CardContent className="p-4">
          <div className="relative aspect-[3/4]">
            ...
            <Image
              ...
              className="object-cover rounded-xl hover:opacity-90 transition-opacity duration-300"
            />
          </div>

          <h3 className="text-xl font-semibold text-gray-800 text-center mb-2 mt-4">
            {product.name}
          </h3>

          <CardDescription className="text-sm text-gray-600 text-center line-clamp-2">
            {product.description}
          </CardDescription>
        </CardContent>
        ...
        <CardFooter className="flex items-center justify-between p-4">
          ...
          <Button
            ...
            onClick={(e) => {
              ...
              // TODO: أضف هنا منطق إضافة المنتج للسلة
            }}
          >
